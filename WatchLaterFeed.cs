...
namespace Lincon
{
    public static class WatchLaterFeed
    {
        public static void HandleFeed(WebApplication app)
        {
            ...
            async Task<string> UseInnerTube(string? accessToken)
            {
                ...
                var payload = new Dictionary<string, object>
                {
                    ...
                    ["params"] = "cAc%3D" // parms access speficic parts of this feed!
                };
            ...
            Tuple<string, int> ExtractData(string json, string access_token, HttpRequest request)
            {
                ...
                // the FEmy_youtube (with parms) feed is layed out like this
                // just rows of gridRenderer with videos of em
                // pretty much the same as history
                ...
                foreach (var item in sectionsRoot.EnumerateArray())
                {
                    if (item.TryGetProperty("tileRenderer", out var tileRenderer))
                    {
                        ...
                        <entry>
                            ...
                            <published>2024-11-30T20:40:12.270Z</published>
                            <updated>2024-11-30T20:40:12.270Z</updated>
            ...
            app.MapGet("/feeds/api/users/default/watch_later", async (HttpRequest request) =>
            {
                try
                {
