...
namespace Lincon
{
    public static class FavouritesFeed
    {   
        public static void HandleFeed(WebApplication app)
        {
            ...
            Tuple<string, int, string, string, int> ExtractData(string json, string access_token, HttpRequest request)
            {
                ...
                // you'll notice that this is almost the same as playlist info
                // well that's because favourites are your liked videos
                // and they're in a playlist!
            ...
            app.MapGet("/feeds/api/users/default/favorites", async (HttpRequest request) =>
            {
                try
                {
